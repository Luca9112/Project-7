...
<section class="herberg-wijzigen">
    ...
    <form action="../includes/reservering-wijzigen.inc.php?reservering=<?php echo $_GET['reservering']; ?>" method="post">
        ...
        <input type="text" name="newAdres" placeholder="Datum" value="<?php echo $row['datum']; ?>"><br>
        ...
        <button type="submit" name="submit-reservering-wijzigen">Bewaren</button>
    ...
    if (isset($_GET["error"])) {
        if ($_GET["error"] == "FormNotSend") {
            ...
            echo '<div class="ErrorMessage">Voer een geldig adres in!</div>';
